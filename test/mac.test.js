...
test('macOUItoNumber - valid', function (t) {
  ...
  for (var m in macs) {
    t.equal(MAC.macOUItoNumber(macs[m][0]), macs[m][1], "MAC number for '" + macs[m][0] + "' is valid");
    t.equal(MAC.macNumberToAddress(macs[m][1]), macs[m][2], "MAC address '" +macs[m][2] + "' is valid");
  }
  t.end();
});
