...
// in the API.
var OPTIONAL_PARAMS = ['belongs_to_type', 'belongs_to_uuid', 'owner_uuid'];
...
function IP(params) {
    ...
    if (
        params.hasOwnProperty('reserved') &&
    ...
    this.use_strings =
        params.use_strings || (params.network && params.network.ip_use_strings);
...
IP.prototype.provisionable = function ipProvisionable() {
    ...
    if (
        this.params.belongs_to_type === 'other' &&
...
function bucketName(networkUUID) {
    return mod_moray.bucketName(
        util.format('napi_ips_%s', networkUUID.replace(/-/g, '_'))
    );
}
