...
function nextFreedIPsonNetwork(opts, network, callback) {
    ...
    var req = opts.app.moray.findObjects(bucket, filter, {
        sort: { attribute: '_mtime', order: 'ASC' },
        limit: 10
    });
...
function nextIPonNetwork(opts, network, callback) {
    ...
    opts.log.debug(
        'nextIPonNetwork: attempting IP allocation on %s network %s',
        network.family,
        network.uuid
    );
    ...
    if (!opts.ipProvisions[network.uuid]) {
        opts.ipProvisions[network.uuid] = new ProvisionInfo(
            opts.baseParams,
            network
        );
    }
    ...
    if (provinfo.tries > constants.IP_PROVISION_RETRIES) {
        opts.log.error(
            { tries: constants.IP_PROVISION_RETRIES },
