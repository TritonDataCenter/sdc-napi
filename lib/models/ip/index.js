...
var CREATE_SCHEMA = {
    ...
    after: [
        function _requireOwningInfo(_opts, _params, validated, callback) {
            var errs = [];
            if (
                validated.hasOwnProperty('belongs_to_uuid') ||
...
function validateNetworkObj(_, name, net, callback) {
    if (!net || typeof net !== 'object') {
        return callback(errors.invalidParam(name, 'could not find network'));
    }
...
function validateNetworkOwner(_opts, _, validated, callback) {
    ...
    if (
        ...
        (!validated.hasOwnProperty('check_owner') || validated.check_owner) &&
        !validated.network.isOwner(validated.owner_uuid)
    ) {
        return callback(
            errors.invalidParam('owner_uuid', constants.OWNER_MATCH_MSG)
        );
    }
...
function getIP(opts, callback) {
    ...
    if (!ip) {
        return callback(
            new restify.InvalidArgumentError('Invalid IP %s', params.ip)
        );
    }
...
function updateIP(app, log, params, callback) {
    ...
    if (params.hasOwnProperty('oldIP')) {
        ...
        if (
            !params.oldIP.owner_uuid &&
            (params.belongs_to_type || params.belongs_to_uuid)
        ) {
    ...
    validate.params(validateParams, null, params, function(
        validationErr,
        validatedParams
    ) {
        if (validationErr) {
...
function createIP(app, log, params, callback) {
    ...
    validate.params(CREATE_SCHEMA, null, params, function(
        validationErr,
        validated
    ) {
        if (validationErr) {
