...
var CREATE_SCHEMA = {
	...
	// XXX KEBE ASKS, validate the UUID?
    ...
    vasync.forEachParallel({
        ...
        func: function _validateNetworkUUID(uuid, cb) {
            mod_net.get({
                ...
                // XXX KEBE ASKS: CHECK MORE STUFF HERE?
                if (router_family === undefined) {
                    router_family = net.family;
                }

                if (net.family !== router_family) {
                    routerTypeNotMatching.push(uuid);
                } else {
                    validated.push(uuid);
                    nets.push(net);
                }
...
function getRouter(app, log, params, callback) {
    ...
    validate.params(GET_SCHEMA, null, params, function (err, validated) {
	    ...
	    function (err2, rec) {
            ...
            // XXX KEBE SAYS FILL ME IN WITH MORE REALITY CHECKS...

            var netUUIDs = rec.value.networks;

            assert.array(netUUIDs, 'router object UUIDs');
		...
		// Return copy of Router for reporting.
		callback(null, new Router(rec.value));
	    });
        });
    });
}
