...
var UPDATE_SCHEMA = {
    ...
    after: function(opts, original, parsed, cb) {
        if (
            !parsed.hasOwnProperty('owner_uuids') &&
...
function provisionableBy(params, uuid) {
    ...
    return (
        params.owner_uuids.concat(constants.UFDS_ADMIN_UUID).indexOf(uuid) !==
        -1
    );
}
...
function _validateNetworks(opts, name, uuids, callback) {
    ...
    if (uuids.length === 0) {
        callback(errors.invalidParam(name, constants.POOL_MIN_NETS_MSG));
        return;
    }
    ...
    vasync.forEachParallel(
        ...
        function(err) {
            ...
            if (poolTypeNotMatching.length !== 0) {
                callback(
                    errors.invalidParam(name, constants.POOL_AF_MATCH_MSG)
                );
                return;
            }
...
function createNetworkPool(app, log, params, callback) {
    ...
    validate.params(CREATE_SCHEMA, { app: app, log: log }, params, function(
        err,
        validatedParams
    ) {
        if (err) {
...
function getNetworkPool(app, log, params, callback) {
    ...
    validate.params(GET_SCHEMA, null, params, function(err, validated) {
        ...
        mod_moray.getObj(app.moray, BUCKET, validated.uuid, function(
            ...
            if (
                validated.provisionable_by &&
                !provisionableBy(rec.value, validated.provisionable_by)
            ) {
                callback(
                    new restify.NotAuthorizedError(constants.msg.POOL_OWNER)
                );
                return;
            }
...
function updateNetworkPool(app, log, params, callback) {
    ...
    getNetworkPool(app, log, params, function(getErr, oldPool) {
        ...
        if (
            !params.hasOwnProperty('networks') &&
        ...
        validate.params(UPDATE_SCHEMA, uopts, params, function(
            err,
            validatedParams
        ) {
            if (err) {
            ...
            if (
                validatedParams.hasOwnProperty('owner_uuids') &&
