...
function validateMACs(opts, name, list, callback) {
    ...
    if (!util_common.hashEmpty(invalid)) {
        ...
        mErr.invalid = Object.keys(invalid).sort();
    ...
    vasync.forEachParallel({
        ...
        func: function (mac, cb) {
            mod_nic.get(opts.app, opts.log, { mac: mac }, function (gErr, res) {
                if (res) {
...
function Aggr(params) {
    ...
    this.params.id = util.format('%s-%s', this.params.belongs_to_uuid,
        this.params.name);
    this.__defineGetter__('id', function () { return this.params.id; });
}
            ...
            if (err2) {
                if (err2.name === 'EtagConflictError') {
                    return callback(new errors.InvalidParamsError(
                        errors.INVALID_MSG, [ errors.duplicateParam('name',
