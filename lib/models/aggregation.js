...
function validateLACPmode(_, name, mode, callback) {
    validate.string(null, name, mode, function(err) {
        ...
        if (LACP_MODES.indexOf(mode) === -1) {
            return callback(
                new errors.invalidParam(
                    name,
                    util.format(
                        'Invalid LACP mode. Supported modes: %s',
                        LACP_MODES.join(', ')
                    )
                )
            );
...
function validateMACs(opts, name, list, callback) {
    ...
    if (typeof list !== 'string' && !util.isArray(list)) {
        return callback(
            new errors.invalidParam(name, 'must be an array of MAC addresses')
        );
    }
    ...
    if (!jsprim.isEmpty(invalid)) {
        callback(
            errors.invalidParam(name, 'invalid MAC addresses', {
    ...
    if (macs.length > constants.MAX_AGGR_MACS) {
        callback(
            errors.invalidParam(
                name,
                util.format(
                    'maximum of %d MAC addresses supported',
    ...
    if (macs.length === 0) {
        callback(
            errors.invalidParam(name, 'must specify at least one MAC address')
        );
        return;
    }
    ...
    vasync.forEachParallel(
        ...
        function(err, res) {
            ...
            if (!jsprim.isEmpty(invalid)) {
                callback(
                    errors.invalidParam(name, 'unknown MAC addresses', {
                        invalid: Object.keys(invalid).sort()
                    })
                );
                return;
            ...
            for (var i = 0; i < nicObjs.length; i++) {
                ...
                if (
                    nicObjs[i].params.belongs_to_uuid !==
                    nicObjs[0].params.belongs_to_uuid
                ) {
                    callback(
                        errors.invalidParam(name, constants.msg.AGGR_MATCH)
                    );
                    return;
                }
            ...
            if (invalidMACs.length !== 0) {
                var serverErr = errors.invalidParam(
                    name,
...
function getAggr(opts, callback) {
    ...
    validate.params(GET_SCHEMA, null, opts.params, function(err, validated) {
        ...
        mod_moray.getObj(opts.app.moray, BUCKET, validated.id, function(
            err2,
            rec
        ) {
            if (err2) {
...
function updateAggr(opts, callback) {
    ...
    getAggr(opts, function(getErr, oldAggr) {
        ...
        validate.params(UPDATE_SCHEMA, opts, opts.params, function(
            err,
            params
        ) {
            if (err) {
...
function deleteAggr(opts, callback) {
    ...
    validate.params(DELETE_SCHEMA, null, opts.params, function(
        vErr,
        validated
    ) {
        if (vErr) {
