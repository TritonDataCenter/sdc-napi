...
function NicTag(params) {
  var errs = [];
  if (!params.name) {
    errs.push(errors.missingParam('name'));
  }

  if (params.name && params.name.replace(NAME_RE, '') !== '') {
    errs.push(errors.invalidParam('name',
      'Name must only contain numbers, letters and underscores'));
  }

  if (errs.length !== 0) {
    throw new errors.InvalidParamsError('Invalid nic tag data: name', errs);
  }
...
function createNicTag(app, log, params, callback) {
  ...
  return getNicTag(app, log, { name: params.name }, function (err, res) {
    if (res) {
      var msg = util.format('A nic tag named "%s" already exists', params.name);
      return callback(new errors.InvalidParamsError(
        msg, [ errors.duplicateParam('name') ]));
    }
