...
function validNameChange(opts, params, parsed, cb) {
    ...
    if (params.oldname !== parsed.name) {
        return getNicTag(opts.app, opts.log, { name: parsed.name }, function(
            err,
            nictag
        ) {
...
function getNicTag(app, log, params, callback) {
    ...
    validate.params(GET_SCHEMA, null, params, function(err) {
        ...
        mod_moray.getObj(app.moray, BUCKET, params.name, function(err2, rec) {
            if (err2) {
                return callback(err2);
            }
    ...
    vasync.pipeline(
        {
            funcs: [
                function _validateUpdate(_, cb) {
                    validate.params(UPDATE_SCHEMA, opts, params, function(
                        err,
                        parsed
                    ) {
                        if (err) {
...
function nicTagsExist(single, opts, name, tags, callback) {
    ...
    if (typeof tags !== 'string' && !util.isArray(tags)) {
        return callback(errors.invalidParam(name, 'must be an array'));
    }
    ...
    if (single) {
        if (tagArr.length === 0) {
            return callback(errors.invalidParam(name, 'must not be empty'));
        }

        if (tagArr.length > 1) {
            return callback(
                errors.invalidParam(name, 'must only specify one nic tag')
            );
        }
    }
    ...
    vasync.forEachPipeline(
        {
            ...
            func: function _getNicTag(tag, cb) {
                getNicTag(opts.app, opts.log, { name: tag }, function(
                    err,
                    res
                ) {
                    if (err || !res) {
