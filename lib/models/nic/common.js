...
function badOwnerUUID(parsedParams, network) {
    var check_owner =
        !parsedParams.hasOwnProperty('check_owner') || parsedParams.check_owner;
    if (
        parsedParams.hasOwnProperty('owner_uuid') &&
        check_owner &&
...
function checkNetwork(parsedParams, name, network) {
    ...
    if (parsedParams.nic_tag === undefined) {
        ...
        return errors.invalidParam(
            name,
            util.format(
                constants.fmt.NIC_TAGS_DIFFER,
                parsedParams.nic_tag,
                network.nic_tag
            )
        );
    }
    ...
    if (parsedParams.vlan_id === undefined) {
        ...
        return errors.invalidParam(
            name,
            util.format(
                constants.fmt.VLAN_IDS_DIFFER,
                parsedParams.vlan_id,
                network.params.vlan_id
            )
        );
    }
    ...
    mod_pool.get(app, log, { uuid: uuid }, function(err2, res) {
        if (err2) {
            if (err2.name === 'ResourceNotFoundError') {
                return callback(
                    errors.invalidParam(name, 'network does not exist')
                );
            }
...
function validateNetwork(opts, name, uuid, callback) {
    ...
    validateNetworkUUID(name, uuid, function(err) {
        ...
        mod_net.get({ app: app, log: log, params: { uuid: uuid } }, function(
            err2,
            res
        ) {
            if (err2) {
    ...
    validateNetwork(opts, name, uuid, function(err, _, toReturn) {
        ...
        if (net.family !== 'ipv4') {
            callback(
                errors.invalidParam(
                    name,
                    util.format(constants.fmt.NET_BAD_AF, 'IPv4')
                )
            );
            return;
        }
...
function validateNetworkParams(opts, _, parsedParams, callback) {
    ...
    if (parsedParams.network_pool) {
        try {
            parsedParams.intersections = getPoolIntersections(
                'network_uuid',
                parsedParams,
                [parsedParams.network_pool]
            );
        } catch (e) {
            callback(e);
            return;
...
function validateUnderlayServer(opts, params, _parsedParams, callback) {
    ...
    if (params.underlay && belongs_to_type !== 'server') {
        callback(
            errors.invalidParam('underlay', constants.SERVER_UNDERLAY_MSG)
        );
    } else {
        callback(null);
    }
...
function listVnetCns(opts, callback) {
    ...
    mod_portolan_moray.vl2LookupCns(opts, function(listErr, cns) {
        if (listErr) {
            opts.log.error(
                { err: listErr, vnet_id: opts.vnet_id },
                'listVnetCns: error fetching cn list on vnet'
            );
            return callback(listErr);
        }

        var vnetCns = Object.keys(
            cns.reduce(function(acc, cn) {
                acc[cn.cn_uuid] = true;
                return acc;
            }, {})
        );
