...
function ipOnNetworkPool(opts, callback) {
    ...
    var haveNetErr = (!opts.err || opts.err.context !=
        mod_ip.bucket(params.network_uuid));
...
function commitBatch(opts, callback) {
    opts.log.debug({ batch: opts.batch }, 'batch: commit');
...
function nicAndIP(opts, callback) {
    ...
    util_common.repeat(function (cb) {
        ...
        if (err) {
            opts.log.error({ err: err, params: opts.params, time: took },
                'Error creating nic');
            return callback(err);
        }

        opts.log.info({ params: params, nic: res.serialize(), time: took },
            'Created nic');
