 ...
 */
Provisioner.prototype.setIntersection = function validIntersection(
    intersection
) {
 ...
 */
Provisioner.prototype.fetchNextIP = function fetchNextIP(
    opts,
    dontStop,
    callback
) {
 ...
 */
NetworkPoolProvision.prototype.setIntersection = function setNewIntersection(
    intersection
) {
    this.ip = null;
    this.network = null;
 ...
 */
NetworkPoolProvision.prototype.nextNetwork = function nextNetworkPool(
    opts,
    callback
) {
    var self = this;
 ...
 */
NetworkPoolProvision.prototype.currentNetFailed = function currentNetFailed(
    err
) {
    if (!err) {
        // No error yet.
...
NetworkPoolProvision.prototype.provision = function provisionPool(
    opts,
    callback
) {
...
function randomMAC(opts, callback) {
    ...
    if (opts.macTries > constants.MAC_RETRIES) {
        opts.log.error(
            ...
            'Could not provision nic after %d tries',
            opts.macTries
        );
...
function nicBatch(opts, callback) {
    ...
    if (opts.shootdownNIC) {
        ...
        opts.batch = opts.batch.concat(
            mod_portolan_moray.vl2CnEventBatch({
                log: opts.log,
                vnetCns: opts.vnetCns,
...
function nicAndIP(opts, callback) {
    ...
    util_common.repeat(
        function(cb) {
            ...
            vasync.pipeline(
                ...
                function(err) {
                    if (err) {
                        opts.log.warn(
                            { err: err, final: err.stop },
                            'error in repeat'
                        );
                        if (
                            err.stop ||
                            retries > constants.NIC_PROVISION_RETRIES
                        ) {
                            // No more to be done:
                            cb(err, null, false);
                            return;
        ...
        function(err, res) {
            ...
            opts.log.info(
                { params: params, obj: res.serialize() },
                'Created nic'
            );
