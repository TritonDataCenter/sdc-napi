...
var UPDATE_SCHEMA = {
    ...
    after: [
        // Only allow IP changes to the first NIC of an aggr
        function checkAggr(opts, _, parsed, cb2) {
            assert.object(opts.existingNic, 'existingNic');
            var oldNic = opts.existingNic;

            // If the IP isn't changing, we're done
            if (!parsed.hasOwnProperty('ip')) {
                cb2();
                return;
            }

            mod_aggr.list({ macs: [ oldNic.mac ]}, function onList(err, aggrs) {
                if (err) {
                    // If not found, NIC isn't in an aggr, otherwise
                    // propagate the error.
                    cb2(err.statusCode === 404 ? null : err);
                    return;
                }
                ...
                if (aggrs[0].macs[0] !== oldNic.mac) {
                    cb2(errors.InUseError(util.fmt('NIC is in use by aggr %s',
                        aggrs[0].id)));
                    return;
                }
