...
// Currently, the immutable VPC fields are the same as the immutable
// fabric fields.
var IMMUTABLE_VPC_FIELDS = IMMUTABLE_FABRIC_FIELDS;
...
var OVERLAP_NORMAL_NET_SQL = OVERLAP_SQL_PREFIX +
    ' AND fabric != true AND vpc != true;';

var OVERLAP_FABRIC_NET_SQL = OVERLAP_SQL_PREFIX +
    ' AND fabric = true AND vnet_id = $3;';

var OVERLAY_VPC_NET_SQL = OVERLAP_SQL_PREFIX +
    ' AND vpc = true AND vnet_id = $3;';
 ...
 *
 * For VPCs, since each owner can potentially have multiple VPCs, we use
 * 'vpc-XXXX' where XXXX is the vnet id (which must be unique across
...
function validateVPCFabric(_opts, _original, parsedParams, cb) {
    // The fabric and vpc options are mutually exclusive
    if (parsedParams.fabric && parsedParams.vpc) {
        cb(errors.invalidParam('vpc', constants.msg.VPC_FABRIC_NETWORK));
        return;
    }

    // If not a VPC, we're done
    if (!parsedParams.vpc) {
        cb();
        return;
    }
...
/**
 * Fabric and vpc networks are only allowed to have a single owner. During
 * network creation, we check that only one owner is specified. Updates to the
 * list are prevented by validateImmutableFields().
 */
...
function validateImmutableFields(opts, _original, parsed, cb) {
    ...
    if (network.fabric) {
        ...
        IMMUTABLE_VPC_FIELDS.forEach(function (field) {
            if (parsed.hasOwnProperty(field)) {
                errs.push(errors.invalidParam(field,
                    constants.msg.VPC_PROP_IMMUTABLE));
            }
        });
    }
...
function validateProvisionRange(opts, _, parsedParams, callback) {
    ...
    if (opts.fabric) {
        ...
        sql = fmt(OVERLAY_VPC_NET_SQL, BUCKET.name);
        args = [
            subnetStart.toString(),
            subnet.toString(),
            parsedParams.vnet_id
...
function createNetworkObject(opts) {
    ...
    // VPCs always have a gateway IP. If none was given, auto-assign one.
    if (params.vpc && !params.gateway) {
        params.gateway = params.provision_start_ip;
    }
...
function validateNicTagExists(opts, name, tags, callback) {
...
function Network(params) {
    ...
    this.vpc = params.vpc || false;

    // Either the network must not be a fabric or a vpc, o
    assert((!this.fabric && !this.vpc) || (this.fabric ^ this.vpc));
...
Network.prototype.raw = function networkRaw(_) {
     ...
     */
    if (!this.params.fabric && !this.params.vpc) {
...
Network.prototype.serialize = function networkSerialize(opts) {
    ...
    if (this.fabric || this.vpc) {
        ser.fabric = this.fabric;
        ser.vpc = this.vpc;
...
function createNetwork(opts, callback) {
    ...
    vasync.pipeline({
        funcs: [
        function _selectSchema(_, cb) {
            // Many of the existing tests expect a warden style validation
            // error. However for VPC networks, there are a few differences
            // in terms of mandatory and optional parameters, so if vpc
            // was specified, we use the VPC_CREATE_SCHEMA instead of the
            // default CREATE_SCHEMA.
            if (!opts.params.hasOwnProperty('vpc')) {
                cb();
                return;
            }

            validate.bool(null, 'vpc', opts.params.vpc,
                function vpcVal(err, val) {
                    if (err) {
                        cb(err);
                        return;
                    }
        ...
        function _validateHttpParams(_, cb) {
            validate.params(schema, copts, opts.params,
        ...
        // We get the VPC since we'll need it to both validate that the
        // network lies within the CIDR range, but also need the CIDR block
        // for the default route table for the network (otherwise we could
        // validate the network range as part of VPC_CREATE_SCHEMA
        function _getVPC(_, cb) {
            if (!validatedParams.vpc) {
                cb();
                return;
            }

            mod_fabric.getVPC(opts, function onGet(err, v) {
                if (err) {
                    cb(err);
                    return;
                }
        ...
        function _validateVPCRange(_, cb) {
            if (!validatedParams.vpc) {
                cb();
                return;
            }
            ...
            vpc.ip4_cidr.forEach(function eachCIDR(cidr) {
                var netTxt = cidr.split('/');
                var net = ipaddr.createCIDR(util_ip.toIPAddr(netTxt[0],
                    netTxt[1]));

                // The CIDR blocks for a VPC should be disjoint, so we can
                // stop checking once we find a CIDR block that contains
                // the subnet we're trying to create.
                if (net.contains(validatedParams.subnet)) {
                    found = true;
                    cb();
                    return;
                }
            ...
            if (!found) {
                cb(new errors.InvalidParam('subnet',
                    fmt(constants.fmt.VPC_SUBNET_INVALID,
                    validatedParams.subnet)));
            }
        },

        function _allocateVpcVlan(_, cb) {
            // Skip for non-VPC networks
            if (!validatedParams.vpc) {
                cb();
                return;
            }

            mod_vlan.createVpcVLAN({
                owner_uuid: validatedParams.owner_uuids[0],
                vpc_uuid: validatedParams.vpc_uuid
            }, function onVlan(err, v) {
                if (err) {
                    cb(err);
                    return;
                }
        ...
        function _createDefaultRouteTbl(_, cb) {
            // We only create a default routing table for vpc subnets, and
            // then only if one was not given.
            if (!validatedParams.vpc || validatedParams.routes) {
                cb();
                return;
            }

            // createNetworkObject() will ensure the gateway address has been
            // assigned.
            assert(validatedParams.gateway, 'validateParams.gateway');

            var routes = {};

            vpc.ip4_cidr.forEach(function eachCIDR(cidr) {
                routes[cidr] = validatedParams.gateway;
            });

            validatedParams.routes = routes;
            cb();
        },

        function _createNet(_, cb) {
            var raw = network.raw();
            var batch = [ network.batch() ];
