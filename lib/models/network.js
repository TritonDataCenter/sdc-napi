...
var OPTIONAL = ['description', 'owner_uuid'];
...
Network.prototype.raw = function networkRaw() {
    ...
    OPTIONAL.concat(['gateway', 'resolvers', 'routes']).forEach(function (opt) {
...
Network.prototype.serialize = function networkSerialize() {
    ...
    if (this.params.routes) {
        ser.routes = {};
        for (r in this.params.routes) {
            var key = r.indexOf('/') !== -1 ?  util_subnet.fromNumberArray(r)
                : util_ip.ntoa(r);
            var val = util_ip.numberToAddress(this.params.routes[r]);
            ser.routes[key] = val;
        }
    }
...
function listNetworks(app, log, params, callback) {
    ...
    if (params.provisionable_by) {
        ...
        params.owner_uuid = [ params.provisionable_by, '!*' ];
