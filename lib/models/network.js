...
function validateParams(params) {
  var required = ['name', 'vlan', 'provision_start_ip', 'provision_end_ip',
    'nic_tag'];
  ...
  for (var p in ipParams) {
    ...
    if (params.hasOwnProperty(ipParam)) {
  ...
  if (params.hasOwnProperty('resolvers')) {
    var resolverIPs = params.resolvers;
...
Network.prototype.randomIPnum = function networkRandomIPnum() {
  return Math.floor(Math.random() *
    Number(this.params.provision_end_ip - this.params.provision_start_ip + 1))
