 ...
 * on fabrics:
 */
var IMMUTABLE_FABRIC_FIELDS = ['gateway', 'owner_uuids'];

var CONTAINING_NET_SQL =
    'SELECT uuid FROM ' +
    BUCKET.name +
...
var CREATE_SCHEMA = {
    ...
    after: [validateFabricOwner, validateProvisionRange, validateNicTagMTU]
};
...
function returnNetworkIfOwner(params, network, callback) {
    if (params.owner_uuid && !network.isOwner(params.owner_uuid)) {
        // If this is a fabric (where owner_uuid is part of the
        // restify path), don't return the network if the user doesn't
        // own it:
        return callback(new restify.ResourceNotFoundError('network not found'));
    }
    ...
    return callback(new restify.NotAuthorizedError(constants.msg.NET_OWNER));
}
...
function provisionRangeUpdates(app, log, network, update, callback) {
    if (
        !update.hasOwnProperty('provision_start_ip') &&
    ...
    if (
        update.hasOwnProperty('provision_end_ip') &&
        update.provision_end_ip.compare(network.params.provision_end_ip) !== 0
    ) {
        toMove.push({
    ...
    function moveRecord(move, cb) {
        ...
        mod_ip.get(getOpts, function(err2, oldRec) {
            ...
            if (!oldRaw.hasOwnProperty('belongs_to_uuid') && !oldRaw.reserved) {
                batch.push({
...
function validateImmutableFields(opts, _original, parsed, cb) {
    ...
    IMMUTABLE_NET_FIELDS.forEach(function(field) {
        ...
        if (network[field] !== parsed[field]) {
            errs.push(
                errors.invalidParam(field, constants.msg.NET_PROP_IMMUTABLE)
            );
        }
    });

    if (
        parsed.hasOwnProperty('subnet') &&
        network.subnet.compare(parsed.subnet) !== 0
    ) {
        errs.push(
            errors.invalidParam('subnet', constants.msg.NET_PROP_IMMUTABLE)
        );
    }
    ...
    IMMUTABLE_FABRIC_FIELDS.forEach(function(field) {
        if (parsed.hasOwnProperty(field)) {
            errs.push(
                errors.invalidParam(field, constants.msg.FABRIC_PROP_IMMUTABLE)
            );
        }
    });
...
function validateProvisionRangeUpdate(opts, params, parsed, cb) {
    if (
        !parsed.hasOwnProperty('provision_start_ip') &&
    ...
    var toValidate = {
        ...
        provision_start_ip:
            parsed.provision_start_ip ||
            params.network.params.provision_start_ip,
        provision_end_ip:
            parsed.provision_end_ip || params.network.params.provision_end_ip
    };
...
function validateProvisionRange(opts, _, parsedParams, callback) {
    ...
    if (
        ...
        errs.push(
            errors.invalidParam(
                'provision_start_ip, provision_end_ip',
    ...
    if (parsedParams.fabric) {
        if (subnetType === 'ipv4' && !util_ip.isRFC1918(subnetStart)) {
            callback(errors.invalidParam('subnet', constants.PRIV_RANGE_ONLY));
            return;
        }
        ...
        if (subnetType !== 'ipv4') {
            callback(errors.invalidParam('subnet', constants.FABRIC_IPV4_ONLY));
            return;
        }
    } else {
        /**
    ...
    var overlapSQL = fmt(
        ...
        opts.owner_uuid
            ? fmt(
                  " AND '%s' = ANY(owner_uuids_arr) AND fabric = true",
                  opts.owner_uuid
              )
            : ' AND fabric != true'
    );
...
function createValidNetwork(opts, callback) {
    ...
    validate.params(CREATE_SCHEMA, copts, params, function(
        ...
        // is unset.
        if (
            validatedParams.fabric &&
            validatedParams.internet_nat &&
...
function _validateNicTagMTU(opts, nic_tag, mtu, callback) {
    return mod_nicTag.get(opts.app, opts.log, { name: nic_tag }, function(
        err,
        nicTag
    ) {
        if (err) {
        ...
        if (nicTag.params.mtu < mtu) {
            return callback(
                errors.invalidParam('mtu', constants.MTU_NETWORK_GT_NICTAG)
            );
        }
    ...
    if (!parsed.hasOwnProperty('nic_tag')) {
        return getNetwork(
            ...
            function(err, network) {
                ...
                return _validateNicTagMTU(
                    opts,
                    network.params.nic_tag,
                    parsed.mtu,
                    callback
                );
            }
...
function Network(params) {
    ...
    var subnet = params.subnet
        ? ipaddr.createCIDR(params.subnet)
    ...
    if (params.hasOwnProperty('name_str')) {
        ...
        this.params.name = params.name_str.substr(
            params.name_str.indexOf(':') + 1
        );
    } else {
    ...
    if (resolvers) {
        this.params.resolvers = util_common
            .arrayify(resolvers)
            .map(util_ip.toIPAddr);
    }
...
Object.defineProperty(Network.prototype, 'family', {
    get: function() {
        return this.params.subnet_type;
    }
});

Object.defineProperty(Network.prototype, 'mtu', {
    get: function() {
        return this.params.mtu;
    }
});
...
Network.prototype.raw = function networkRaw(_) {
    var raw = {
        ...
        provision_end_ip_addr: this.params.provision_end_ip.toString(),
        provision_start_ip_addr: this.params.provision_start_ip.toString(),
    ...
    if (this.params.subnet_type === 'ipv4') {
        ...
        raw.subnet_end_ip =
            raw.subnet_start_ip + Math.pow(2, 32 - raw.subnet_bits) - 1;
    }
...
Network.prototype.isOwner = function networkHasOwner(owner) {
    if (
        !this.params.hasOwnProperty('owner_uuids') ||
    ...
    return (
        this.params.owner_uuids
            .concat(constants.UFDS_ADMIN_UUID)
            .indexOf(owner) !== -1
    );
};
            ...
            funcs: [
                ...
                function _createIPs(_, cb) {
                    ...
                    if (network.params.gateway) {
                        if (
                            network.fabric &&
                            !network.params.internet_nat &&
                            network.params.owner_uuids
                        ) {
                            ipsToCreate[
                                network.params.gateway.toString()
                            ] = userReservedIP(
                                network,
                                network.params.gateway,
                    ...
                    for (var r in network.params.resolvers) {
                        ...
                        if (
                            network.subnet.contains(num) &&
                            !ipsToCreate.hasOwnProperty(num.toString())
                        ) {
                            ipsToCreate[num.toString()] = adminReservedIP(
                                network,
                                num,
                                app.config.ufdsAdminUuid
                            );
                    ...
                    if (network.family === 'ipv4') {
                        ...
                        if (!ipsToCreate.hasOwnProperty(maxIP.toString())) {
                            ipsToCreate[maxIP.toString()] = adminReservedIP(
                                network,
                                maxIP,
                                app.config.ufdsAdminUuid
                            );
                    ...
                    // under/overflow
                    var lowerBound = util_ip.ipAddrMinus(
                        network.provisionMin,
                        1
                    );
                    var upperBound = util_ip.ipAddrPlus(
                        network.provisionMax,
                        1
                    );
                    [lowerBound, upperBound].forEach(function(rangeNum) {
                        if (!ipsToCreate.hasOwnProperty(rangeNum)) {
                            ipsToCreate[rangeNum] = placeholderIP(
                                network,
                                rangeNum
                            );
                        }
...
function updateNetwork(opts, callback) {
    ...
    validate.params(UPDATE_SCHEMA, uopts, params, function(err, validated) {
        ...
        if (validated.hasOwnProperty('owner_uuids')) {
            batch[0].value.owner_uuids = mod_moray.arrayToVal(
                validated.owner_uuids
            );
        ...
        provisionRangeUpdates(app, log, params.network, validated, function(
            err2,
            updates
        ) {
            if (err2) {
...
function getNetwork(opts, callback) {
    ...
    validate.params(GET_SCHEMA, null, params, function(valErr, validated) {
        ...
        mod_moray.getObj(app.moray, BUCKET, validated.uuid, function(
            err2,
            rec
        ) {
            if (err2) {
            ...
            return returnNetworkIfOwner(
                validated,
                new Network(rec.value),
                callback
            );
        });
    });
    ...
    vasync.pipeline(
        {
             ...
             */
                ...
                function deleteNetBucket(_, cb) {
                    ...
                    app.moray.delBucket(bucketName, function postBucketDel(
                        err
                    ) {
                        if (
                            err &&
