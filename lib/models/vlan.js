...
var VALID_FIELDS = ['description', 'name', 'owner_uuid', 'vlan_id', 'vnet_id'];
...
Object.defineProperty(FabricVLAN.prototype, 'id', {
    get: function() {
        return morayVlanKey(this.params);
    }
});
        ...
        app.moray.batch(batch, function(bErr) {
            if (bErr) {
                ...
                if (VError.hasCauseWithName(bErr, 'EtagConflictError')) {
                    callback(
                        new errors.InUseError(constants.msg.VLAN_USED, [
...
function deleteFabricVLAN(opts, callback) {
    ...
    validate.params(DELETE_SCHEMA, null, opts.params, function(err, validated) {
        if (err) {
            return callback(err);
        }
...
function updateFabricVLAN(opts, callback) {
    ...
    validate.params(UPDATE_SCHEMA, null, opts.params, function(
        vErr,
        validated
    ) {
        if (vErr) {
