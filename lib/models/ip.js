...
function validateNetworkOwner(params, validated, callback) {
  ...
  if (validated.network.params.owner_uuid && validated.owner_uuid &&
    validated.network.params.owner_uuid !== validated.owner_uuid &&
    validated.owner_uuid !== constants.UFDS_ADMIN_UUID) {
...
function provisionFreedIP(app, log, params, callback) {
  ...
  req.once('end', function () {
    if (!objs || objs.length === 0) {
      return callback(new errors.SubnetFullError(constants.SUBNET_FULL_MSG));
    }

    util_common.repeat(function (cb) {
      var ipRec = objs.shift();
      if (!ipRec) {
        return cb(new errors.SubnetFullError(constants.SUBNET_FULL_MSG));
      }
...
function getIP(app, log, params, callback) {
  ...
  var ipBucket = getBucketObj(params.network_uuid);
  mod_moray.getObj(app.moray, ipBucket, ipNum.toString(), function (err, rec) {
    if (err) {
  ...
  validate.params(validateParams, function (validationErr) {
    ...
    provisionNewIP(app, log, params, function (err, res) {
      ...
      provisionFreedIP(app, log, params, function (err2, res2) {
        ...
        if (!res2) {
          return new errors.SubnetFullError(constants.SUBNET_FULL_MSG);
        }
