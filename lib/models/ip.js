 ...
 * bucket is created with certain keys pre-populated:
 *
 *   +--------------------------------------------------------+
 *   |   | GW |     | PS |                | PE |         | BR |
 *   +--------------------------------------------------------+
 ...
 *
 * How PS and PE are used is explained in the "How provisionNewIP() works"
 * section below.
 *
 *
 ...
 *    detection much easier (explained in the "How provisionNewIP() works"
 *    section below).
 *
 ...
 *    provisionNewIP() below).
 *
 * 2) If there are no never-used IPs, return the one that was freed the
 *    longest time ago (this is provisionFreedIP() below).
 *
 ...
 *
 *
 * ## How provisionNewIP() works
 *
 * provisionNewIP() uses Postgres' lead() window function to search for gaps
 * in the values of the ip column for the napi_ips_<uuid> bucket for that
 ...
 * for the second.
 *
 * provisionNewIP() tries up to 10 times to find a new gap_start, and then
 ...
 * reserved: false, and no other values.  This ensures that provisionNewIP()
 * can still continue to find gaps in the IP range efficiently.
 *
 ...
 * longest time ago.
 *
 * As with provisionNewIP(), we retrieve up to 10 records, trying a put on
 ...
 * requesting a specific IP.  It moves through the provisionNewIP() and
 * provisionFreedIP() paths when we don't care about which IP a VM gets -
 ...
 *            |                  |                   |
 *            |                  |                   |
 *        updateIP()         deleteIP()       provisionFreedIP()
 *            |                  |                   |
 *            |                  |                   |
 *            |                  |                   |
 *            |                  v                   |
    ...
    return util_common.repeat(function (cb) {
        ...
        req.once('end', function () {
            ...
            addIP(app, log, params, function (err, res) {
                ...
                if (res) {
                    log.info(res,
                        'provisionNewIP: provisioned IP after %d tries',
                        tries);
                    return cb(null, res);
                }
        ...
        if (err) {
            log.error(err,
                'provisionNewIP: unable to provision IP after %d tries', tries);
        }
    ...
    req.once('end', function () {
        ...
        util_common.repeat(function (cb) {
            ...
            app.moray.putObject(bucket.name, ipRec.key, newIP.raw(),
                ...
                log.info({ original: ipRec, newIP: newIP.raw() },
                    'provisionFreedIP: provisioned IP after %d tries', tries);
                return cb(null, newIP);
            });
        }, function (err, res) {
            if (err) {
                log.error(err,
                    'provisionFreedIP: unable to provision IP after %d tries',
                    tries);
            }
...
function IP(params) {
    ...
    this.__defineGetter__('address', function () {
        return util_ip.numberToAddress(this.params.ip);
    });
}
...
function getIP(app, log, params, callback) {
    ...
    mod_moray.getObj(app.moray, ipBucket, ipNum.toString(),
        ...
        var ip = new IP(rec.value);
        ip.params.network_uuid = params.network_uuid;
        ip.params._id = rec._id;
        ip.params._etag = rec._etag;
        return callback(null, ip);
    });
}
...
function updateIP(app, log, params, callback) {
    ...
    validate.params(validateParams, function (validationErr, validatedParams) {
        ...
        if (params.unassign) {
            ...
            log.info({ ip: updateOpts.key, val: updateOpts.val },
                'updateIP: Unassigning IP %s', util_ip.aton(updateOpts.key));
        }
