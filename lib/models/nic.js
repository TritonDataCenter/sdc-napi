...
function validateNetworkParams(app, log, params, parsedParams, callback) {
    ...
    if (parsedParams.network && parsedParams.owner_uuid &&
        parsedParams.network.params.owner_uuid &&
        (parsedParams.network.params.owner_uuid !== parsedParams.owner_uuid) &&
        parsedParams.owner_uuid !== constants.UFDS_ADMIN_UUID) {
...
function createFromRaw(app, log, params, callback) {
    ...
    if (params.hasOwnProperty('nic_tags_provided')) {
        if (params.nic_tags_provided === ',,') {
                delete params.nic_tags_provided;
        } else {
                params.nic_tags_provided =
                    /* JSSTYLED */
                    params.nic_tags_provided.replace(/^,/, '').replace(/,$/, '');
        }
    }
...
/**
 * Returns the raw nic_tags_provided format
 */
function rawNicTags(arr) {
    return ',' + arr.join(',') + ',';
}
...
Nic.prototype.raw = function nicRaw() {
    ...
    if (this.params.hasOwnProperty('nic_tags_provided')) {
        raw.nic_tags_provided = rawNicTags(this.params.nic_tags_provided);
    }
                ...
                validate.params({
                    ...
                    if (updateParams &&
                        ...
                        updateParams.nic_tags_provided =
                                rawNicTags(updateParams.nic_tags_provided);
                    }
