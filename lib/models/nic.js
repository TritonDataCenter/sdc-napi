 ...
 */
function ensureNicsExist(app, log, name, tags, callback) {
  var errTags = [];
        ...
        validate.params({
          ...
          optional: {
            ...
            network_uuid: function (name, uuid, cb2) {
              validate.UUID(name, uuid, function (err) {
                if (err) {
                  return cb2(err);
                }
            ...
            nic_tags_provided: function (name, tags, cb2) {
              ensureNicsExist(app, log, name, tags, cb2);
            },
    ...
    funcs: [
      function _provisionIP(_, cb) {
        ...
        return provisionIP(app, log, params, function (err, res) {
          ...
          updateParams.networkuuid = ip.params.network_uuid;
      ...
      function _updateNic(_, cb) {
        ...
        return app.ufds.update({
          ...
          id: 'mac=' + macNum,
          params: updateParams,
          createFunc: function (p, cb2) { createFromRaw(app, log, p, cb2); }
        }, function (err, res) {
          if (err) {
            return cb(err);
          }

          log.debug(res.serialize(), 'nic "%s" updated', params.mac);
          updatedNic = res;
