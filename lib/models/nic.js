...
function provisionIPonNetworkPool(app, log, params, callback) {
    ...
    function tryNetProvision() {
        ...
        mod_net.get(app, log, { uuid: nextUUID }, function (err, res) {
            ...
            return provisionIPonNetwork(app, log, params,
                function (err2, res2) {
                if (err2) {
 ...
 */
function createMorayNic(app, log, mac, raw, callback) {
    app.moray.putObject(BUCKET.name, mac.toString(), raw, { etag: null },
        function (err) {
 ...
 */
function provisionNic(app, log, startAt, raw, callback) {
    var tries = 0;
    var num = startAt;
    ...
    util_common.repeat(function (cb) {
        ...
        createMorayNic(app, log, num, raw, function (err, res) {
            if (err) {
                if (err.duplicate) {
                    num++;
...
function Nic(params) {
    ...
    assert.optionalString(params.status, 'status');
        ...
        funcs: [
                ...
                var toValidate = {
                    ...
                    optional: {
                        ...
                        nic_tags_provided:
                            mod_nicTag.validateExists.bind(null, app, log,
                                false),
                        // XXX: only allow one of the nics for a belongs_to to
                        // be primary
            ...
            function _ip(_, cb) {
                ...
                if (validated.network_pool) {
                    ...
                    provFn = validated.ip ? provisionSpecificIP :
                        provisionIPonNetwork;
                }
            ...
            function _createProvisionNic(_, cb) {
                ...
                if (haveMAC) {
                    ...
                    return createMorayNic(app, log, nic.mac, nic.raw(), cb);
                }

                // No MAC requested a MAC - generate a random one and retry
                // if it's taken
                provisionNic(app, log, nic.mac, nic.raw(),
                    function (err, mac) {
                    if (err) {
                        return cb(err);
                    }
    ...
    vasync.pipeline({
        funcs: [
            ...
            function _provisionIP(_, cb) {
                ...
                var provFn = provisionIPonNetwork;

                if (oldNic && oldNic.params.ip && params.ip) {
                    provFn = provisionSpecificIP;
            ...
            function _updateNic(_, cb) {
                ...
                Object.keys(updateParams).forEach(function (p) {
                    ...
                    if (BOOL_PARAMS.indexOf(p) !== -1 && !updateParams[p]) {
            ...
            function _updateIP(_, cb) {
                ...
                if (updateParams.ip && (updateParams.ip !== oldNic.params.ip) &&
                    ...
                    return mod_ip.del(app, log, delParams, function (err) {
                        ...
                        log.debug(updateParams, 'updateNic: old IP deleted');
                        return cb();
                    });
                }
        ...
        function _delIP(_, cb) {
            ...
            if (nic.ip.params.belongs_to_uuid !== nic.params.belongs_to_uuid) {
                log.debug({ mac: params.mac, ip: nic.ip.address },
                    'deleteNic: IP and nic belongs_to_uuid do not match');
                return cb();
            }
