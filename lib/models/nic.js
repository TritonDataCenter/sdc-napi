...
function createFromRaw(app, log, params, callback) {
  ...
  // UFDS will return a scalar if there's only one
  if (properParams.hasOwnProperty('nic_tags_provided') &&
      typeof (properParams.nic_tags_provided) !== 'object') {
    properParams.nic_tags_provided = [properParams.nic_tags_provided];
  }
    ...
    return mod_net.list(app, log, query, function (err, res) {
      ...
      if (res.length === 0) {
        return callback(new restify.InvalidArgumentError(
          'No networks found matching parameters: vlan=%d, nic_tag=%s',
          params.vlan, params.nic_tag));
      }
      if (res.length != 1) {
        return callback(new restify.InvalidArgumentError(
          'Too many networks found matching parameters: vlan=%d, nic_tag=%s',
          params.vlan, params.nic_tag));
      }
...
Nic.prototype.serialize = function nicSerialize() {
  ...
  if (this.network) {
    ...
    var netParams = ['netmask', 'gateway', 'vlan', 'nic_tag', 'resolvers'];
