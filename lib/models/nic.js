...
function findFreeIPbyAddress(app, netUUID, addr) {
  ...
  if (net.ips.reserved.hasOwnProperty(ip)) {
    return new restify.InvalidArgumentError("IP address '%s' is already taken", addr);
  }
...
function matchNic(nic, params) {
  ...
  for (var p in validParams) {
    var param = validParams[p];
    if (!params.hasOwnProperty(param)) {
      continue;
    }

    var nicParam = nic.property(param);
    var params = params[param].split(',');
    var match = false;
    for (var p in params) {
      var param = params[p];
      if (param == nicParam) {
