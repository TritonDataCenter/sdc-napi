...
function loadNetwork(app, name, netData, callback) {
  ...
  if (missing.length !== 0) {
    var reqErr = new errors.InvalidParamsError('Missing parameters', missing);
...
NAPI.prototype.createMorayClient = function createMorayClient() {
  ...
  conf.log = self.log.child({ component: 'moray'});
...
NAPI.prototype.init = function serverInit() {
  ...
  models.init(self, function (err) {
    if (err) {
      // XXX: retry until this succeeds
      self.log.error('Error initializing models');
    } else {
