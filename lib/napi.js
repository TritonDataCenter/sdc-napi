...
function periodicUsageLog(log) {
    log.info({ memory: process.memoryUsage() }, 'Current memory usage');
}
...
function NAPI(opts) {
    ...
    server.on('after', function _filteredAuditLog(req, res, route, err) {
        ...
        restify.auditLogger({
            ...
            body: !(
                req.method === 'GET' && Math.floor(res.statusCode / 100) === 2
            )
        })(req, res, route, err);
    });
    ...
    function modelInitRetry() {
        models.init(self, function(err) {
            ...
            if (err) {
                self.log.error(
                    err,
                    'Error initializing models (attempt=%d)',
                    att
                );
                att++;
