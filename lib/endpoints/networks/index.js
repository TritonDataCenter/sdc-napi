...
function ensureNetworkUnused(req, res, next) {
    mod_pool.list(req.app, req.log, { networks: [req.params.uuid] }, function(
        err,
        usedBy
    ) {
        if (err) {
...
function register(http, before) {
    ...
    http.post(
        { path: '/networks', name: 'CreateNetwork' },
        before,
        postNetwork
    );
    http.get({ path: '/networks', name: 'ListNetworks' }, before, listNetworks);
    http.head(
        { path: '/networks', name: 'HeadNetworks' },
        before,
        listNetworks
    );

    http.get(
        { path: '/networks/:uuid', name: 'GetNetwork' },
        before,
        getNetwork
    );
    http.head(
        { path: '/networks/:uuid', name: 'HeadNetwork' },
        before,
        getNetwork
    );

    http.put(
        { path: '/networks/:uuid', name: 'PutNetwork' },
        netRequired,
        putNetwork
    );

    http.del(
        { path: '/networks/:uuid', name: 'DeleteNetwork' },
        before.concat(ensureNetworkUnused),
        deleteNetwork
    );
}
