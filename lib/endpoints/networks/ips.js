...
function validateIP(req, res, next) {
    ...
    if (!ip) {
        return next(new restify.ResourceNotFoundError('Invalid IP address'));
    }
    ...
    if (!req._network.subnet.contains(ip)) {
        return next(new restify.ResourceNotFoundError('IP is not in subnet'));
    }
...
function putIP(req, res, next) {
    ...
    mod_ip.get({ app: req.app, log: req.log, params: req.params }, function(
        err,
        ip
    ) {
        if (err || !ip) {
            ...
            return mod_ip.create(req.app, req.log, req.params, function(
                err2,
                ip2
            ) {
                if (err2) {
