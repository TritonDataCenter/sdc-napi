...
function usage() {
    ...
    console.log('ip-search <IP address>');
    console.log('');
...
/*
 * Searches for IP in a network
 */
function searchIPs(napi, opts) {
    var ipAddr = opts.argv.remain[1];
    if (!ipAddr) {
        exit('Error: must supply IP address!');
    }

    var params = getKeyValParams(opts, 2);
    napi.searchIPs(ipAddr, params, standardHandler);
}
