...
/**
 * Calls callback with the appropriate error depending on the contents of
 * errs and invalid
 */
function errResult(errs, invalid, validated, callback) {
  if (errs.length !== 0) {
    return callback(new errors.InvalidParamsError(
      invalid ? 'Invalid parameters' : 'Missing parameters',
      errs.sort(function (a, b) { return a.field > b.field; })));
  }
...
function validateParams(opts, callback) {
  ...
  vasync.forEachParallel({
    ...
    if (opts.hasOwnProperty('after')) {
      return opts.after(validatedParams, function (err) {
        if (err) {
          invalid = true;
          errs = errs.concat(
        ...
        return errResult(errs, invalid, validatedParams, callback);
      });
    }
