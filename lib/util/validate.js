        ...
        if (val.length > constants.MAX_INTERFACE_LEN) {
            return callback(
                errors.invalidParam(
                    name,
                    fmt(
                        'must not be longer than %d characters',
...
function validateIParray(_, name, arr, callback) {
    ...
    if (!util.isArray(arr) && typeof arr !== 'string') {
        return callback(
            new errors.invalidParam(name, constants.msg.ARRAY_OF_STR)
        );
    }
...
function validateMAC(_, name, addr, callback) {
    ...
    try {
        ...
        callback(errors.invalidParam(name, 'invalid MAC address'));
        return;
    }
...
function validateMACarray(_, name, val, callback) {
    ...
    var macErr = errors.invalidParam(
        name,
        fmt('invalid MAC address%s', errs.length === 1 ? '' : 'es')
    );
    macErr.invalid = errs;
...
module.exports = {
    ...
    nicTagMTU: validateMTU(
        constants.MTU_NICTAG_MIN,
        constants.MTU_NICTAG_INVALID_MSG
    ),
    networkMTU: validateMTU(
        constants.MTU_NETWORK_MIN,
