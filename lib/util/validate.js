...
var VALID_STATES = ['provisioning', 'installed', 'running'];
...
function errResult(errs, validated, callback) {
    ...
    if (errs.length !== 0) {
        ...
        return callback(new errors.InvalidParamsError(
            invalid ? errors.INVALID_MSG : 'Missing parameters', sortedErrs));
    }
...
/**
 * Validates the status is one of a limited set of strings.
 */
function validateStatus(name, status, callback) {
