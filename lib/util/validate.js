...
function validateSubnetArray(name, arr, type, callback) {
    var errs = [];
    var subnets = [];
    ...
    util_common.arrayify(arr).forEach(function (i) {
        ...
        if ((type === 'ipv4' && !validIPv4SubnetBits(params)) ||
            (type === 'ipv6' && !validIPv6SubnetBits(params))) {
            errs.push(i);
            return;
        }
    ...
    if (errs.length !== 0) {
        var subnetErr = errors.invalidParam(name,
            fmt('invalid CIDR ranges'));

        subnetErr.invalid = errs;
        callback(subnetErr);
        return;
    }
...
function validateSubnetIPv4Array(_, name, arr, callback) {
    return (validateSubnetArray(name, arr, 'ipv4', callback));
}

function validateSubnetIPv6Array(_, name, arr, callback) {
    return (validateSubnetArray(name, arr, 'ipv6', callback));
}
...
module.exports = {
    ...
    subnetIPv4Array: validateSubnetIPv4Array,
    subnetIPv6Array: validateSubnetIPv6Array,
    UUID: warden.uuid,
