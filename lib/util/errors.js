...
function VPCQuotaExceededError(amt, limit) {
    assert.number(amt, 'amt');
    assert.number(limit, 'limit');

    var msg = util.format(constants.fmt.VPC_QUOTA_EXCEEDED, amt, limit);

    restify.RestError.call(this, {
        restCode: 'QuotaExceeded',
        statusCode: 507,
        message: msg,
        body: {
            code: 'QuotaExceeded',
    ...
    this.name = 'VPCQuotaExceededError';
    this.stop = true;
}

util.inherits(VPCQuotaExceededError, restify.RestError);

function VPCFullError(vpc_uuid) {
    assert.uuid(vpc_uuid, 'vpc_uuid');

    restify.RestError.call(this, {
        restCode: 'VPCFull',
        statusCode: 507,
        message: constants.VPC_FULL_MSG,
        body: {
            code: 'VPCFull',
            message: constants.VPCFULL_MSG,
            vpc_uuid: vpc_uuid
        }
    });

    this.name = 'VPCFullError';
    this.vpc_uuid = vpc_uuid;
    this.stop = true;
}

util.inherits(VPCFullError, restify.RestError);
