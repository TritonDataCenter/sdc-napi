...
var MESSAGES = {
    ...
    LIMIT:
        'invalid limit, must be an integer greater than 0 or less than or ' +
        'equal to 1000',
    ...
    NO_POOL_INTERSECTION:
        'Specified pools do not contain networks ' +
        'that can be used together',
    OBJ: 'must be an object',
    OFFSET:
        'invalid value, offset must be an integer greater than or ' +
        'equal to 0',
    ...
    PROV_TYPES_MISMATCH:
        'provision_start_ip and provision_end_ip must be ' +
        'both IPv4 or both IPv6 addresses',
    PROV_START_TYPE_MISMATCH:
        'provision_start_ip and subnet must both be ' +
        'IPv4 or IPv6 addresses',
    PROV_END_TYPE_MISMATCH:
        'provision_end_ip and subnet must both be ' + 'IPv4 or IPv6 addresses',
...
var FORMAT_MESSAGES = {
    ...
    IP_NONET:
        'No networks matching this nic_tag ("%s") and vlan_id (%d) ' +
        'contained the IP address %s',
    IP_MULTI:
        'Multiple (overlapping) networks (%s) matching this nic_tag ' +
    ...
    POOL_FAILS_CONSTRAINTS:
        'No networks in network pool %s match ' + 'provisioning constraints',
    POOL_FULL_MSG: 'All networks in pool %s are full',
    POOL_NIC_TAGS_AMBIGUOUS: 'Network pool %s has mixed nic_tag values',
    NIC_TAGS_DIFFER:
        'The nic_tag of all specified IPv4 and IPv6 networks ' +
        'must match (have "%s" and "%s")',
    VLAN_IDS_DIFFER:
        'The vlan_id of all specified IPv4 and IPv6 networks ' +
...
module.exports = {
    ...
    MTU_NICTAG_INVALID_MSG:
        'mtu must be a number between ' + MTU_NICTAG_MIN + ' and ' + MTU_MAX,
    MTU_NETWORK_INVALID_MSG:
        'mtu must be a number between ' + MTU_NETWORK_MIN + ' and ' + MTU_MAX,
    ...
    POOL_MIN_NETS_MSG: 'network pool must contain at least one network',
    ...
    PROV_RANGE_ORDER_MSG: 'provision_start_ip must be before provision_end_ip',
